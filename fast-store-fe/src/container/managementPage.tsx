    ...
    return (
        <div className="space-y-2">
            ...
            <div className="w-full p-2 space-y-2 bg-white rounded-md">
                <FormMsg isError={apiState.isError} isLoading={apiState.isLoading} message={apiState.message} errorMessage={apiState.errorMessage} />
                {products.map((item) => (
                    <UpdateProductItem key={item.id} value={item} />
                ))}
            </div>
        </div>
    );
